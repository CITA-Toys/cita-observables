...
class CITAObservables {
    constructor({ server, interval = 1000, reservedRecords = 10, }) {
        this.newBlockNumber = (interval) => rxjs_1.Observable.interval(interval || this.interval)
            .startWith(0)
        ...
        this.newBlockByNumber = (interval) => this.newBlockNumber(interval || this.interval).switchMap(blockNumber => {
            if (typeof blockNumber === 'string') {
                return this.blockByNumber(blockNumber);
            }
            return rxjs_1.Observable.of(new Error('Invalid BlockNumber'));
        });
        this.peerCount = (interval) => rxjs_1.Observable.interval(interval || this.interval)
            .startWith(0)
